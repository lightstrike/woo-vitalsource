<?php
/**
 * Setting registry. Manages WooVitalSource Settings API.
 *
...
class Settings_Registry {
	...
	/**
	 * Returns the admin page settings sections.
	 *
	 * @return array
	 */
	...
	/**
	 * Returns the admin page settings fields.
	 *
	 * @return array
	 */
	...
	public function register_field( string $section, array $field ) {
		 ...
		 */
		$field = apply_filters( 'woo_vitalsource_setting_field_config', $field_config, $field_name, $section );
	...
	public function admin_init() {
		...
		do_action( 'woo_vitalsource_init_settings', $this );
		 ...
		 */
		$setting_sections = apply_filters( 'woo_vitalsource_settings_sections', $this->settings_sections );
		...
		foreach ( $setting_sections as $id => $section ) {
			...
			if ( isset( $section['desc'] ) && ! empty( $section['desc'] ) ) {
				...
				$callback        = function () use ( $section ) {
					echo wp_kses( str_replace( '"', '\"', $section['desc'] ), $this->get_allowed_wp_kses_html() );
				};
	 ...
	 * @return string
	 */
	private function get_field_description( array $args ): string {
		if ( ! empty( $args['desc'] ) ) {
	...
	public function callback_html( array $args ) {
		echo wp_kses( $this->get_field_description( $args ), $this->get_allowed_wp_kses_html() );
	}
	...
	public function callback_file( array $args ) {
		...
		$label = isset( $args['options']['button_label'] ) ? $args['options']['button_label'] : __( 'Choose File', 'woo-vitalsource' );
	...
	public function callback_pages( array $args ) {
		...
		foreach ( $dropdown_args as $key => $arg ) {
			$clean_args[ $key ] = wp_kses( $arg, $this->get_allowed_wp_kses_html() );
		}
		echo wp_dropdown_pages( $clean_args ); //@codingStandardsIgnoreLine
	}
			...
			<?php foreach ( $this->settings_sections as $id => $form ) { ?>
				<div id="<?php echo esc_attr( $id ); ?>" class="group" style="display: none;">
					<form method="post" action="options.php">
						<?php
						do_action( 'woo_vitalsource_settings_form_top', $form );
						settings_fields( $id );
						do_settings_sections( $id );
						do_action( 'woo_vitalsource_settings_form_bottom', $form );
	 ...
	 * @return array
	 */
	public function get_allowed_wp_kses_html() {
		$allowed_atts = [
			'align'      => [],
			'class'      => [],
			'type'       => [],
